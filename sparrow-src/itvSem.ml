...
let eval_uop : Spec.t -> Cil.unop -> Val.t -> Cil.location -> e:ExpArg.t -> n_info:string -> Val.t
  = fun spec u v loc ~e ~n_info ->
    if Val.eq v Val.bot then Val.bot |> Val.modify_footprints [%here] loc e ~n_info else
  ...
  = fun pid lv mem loc ~n_info ->
    let v, here, fpo =
      ...
      | Cil.Var vi ->
        ...
        (Val.of_pow_loc (PowLoc.singleton l), here, None)
      | Cil.Mem e ->
...
let rec model_strcpy mode spec node pid es (mem, global) loc =
  ...
  | (Lval dst)::(StartOf src)::[] | (StartOf dst)::(Lval src)::[] ->
    ...
    let newv = Val.modify_footprints'' here (src_fp :: dst_fp :: []) ~isPointer:true loc (Fun ("strcpy", es)) ~n_info newv in
